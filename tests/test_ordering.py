...
def test_extract_key_order_flattens_nested_sections() -> None:
...
def test_extract_key_order_merges_list_entry_keys() -> None:
    orders = extract_key_order({"spec": {"deps": [{"a": 1}, {"b": 2, "a": 3}]}})

    assert orders["spec.deps"] == ["a", "b"]


def test_spec_keys_follow_template(tmp_path: Path, config_dir: Path, yaml: YAML) -> None:
...
def test_schema_comment_and_document_start_survive(tmp_path: Path, config_dir: Path) -> None:
    target = tmp_path / "ks.yaml"
    _ = target.write_text(KS, encoding="utf-8")
...
def test_already_sorted_file_is_untouched(tmp_path: Path, config_dir: Path) -> None:
    target = tmp_path / "ks.yaml"
    _ = target.write_text(KS, encoding="utf-8")
...
def test_anchors_and_aliases_survive(tmp_path: Path, config_dir: Path) -> None:
    target = tmp_path / "helmrelease.yaml"
    _ = target.write_text(
        "---\n"
        "apiVersion: helm.toolkit.fluxcd.io/v2\n"
...
def test_quoted_scalars_keep_their_quotes(tmp_path: Path, config_dir: Path) -> None:
    target = tmp_path / "ks.yaml"
    _ = target.write_text(
        KS.replace("  interval: 10m\n", '  interval: 10m\n  quoted: "1"\n'),
        encoding="utf-8",
...
def test_file_permissions_are_preserved(tmp_path: Path, config_dir: Path) -> None:
    target = tmp_path / "ks.yaml"
    _ = target.write_text(KS, encoding="utf-8")
    target.chmod(0o640)
...
def test_multi_document_file_sorts_every_document(tmp_path: Path, config_dir: Path) -> None:
    target = tmp_path / "ks.yaml"
