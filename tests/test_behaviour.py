...
from yamlsorter import FileTypeDetector, Outcome, SortingTool, YAMLValue, main
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"kind": "HelmRelease", "spec": {"chartRef": {"name": "app-template"}}},
            "helmrelease-apptemplate",
        ),
        (
            {"kind": "HelmRelease", "spec": {"chart": {"spec": {"chart": "cilium"}}}},
            "helmrelease-cilium",
        ),
...
def test_unsupported_document_is_skipped_not_failed(tmp_path: Path, config_dir: Path) -> None:
...
def test_unparseable_file_fails_without_writing(tmp_path: Path, config_dir: Path) -> None:
...
def test_missing_config_dir_is_reported(tmp_path: Path) -> None:
    tool = SortingTool(tmp_path / "absent")

    assert tool.run([tmp_path], ["ks.yaml"], audit=False) == 2


def test_empty_config_dir_is_reported(tmp_path: Path) -> None:
...
def test_directory_walk_only_picks_up_named_files(tmp_path: Path, config_dir: Path) -> None:
    (tmp_path / "app").mkdir()
...
def test_explicit_file_argument_bypasses_the_name_filter(tmp_path: Path, config_dir: Path) -> None:
...
def test_hidden_directories_are_not_walked(tmp_path: Path, config_dir: Path) -> None:
    (tmp_path / ".git").mkdir()
...
def test_check_mode_reports_without_writing(tmp_path: Path, config_dir: Path) -> None:
    target = tmp_path / "ks.yaml"
...
def test_check_mode_passes_on_sorted_files(tmp_path: Path, config_dir: Path) -> None:
    target = tmp_path / "ks.yaml"
    _ = target.write_text(KS, encoding="utf-8")
...
def test_missing_template_type_is_skipped_rather_than_failing(
...
def test_cli_exit_code_on_check(tmp_path: Path, config_dir: Path) -> None:
    target = tmp_path / "ks.yaml"
    _ = target.write_text(KS, encoding="utf-8")
...
def test_a_header_comment_does_not_block_the_rewrite(tmp_path: Path, config_dir: Path) -> None:
