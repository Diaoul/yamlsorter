    ...
    def _order_for(orders: dict[str, list[str]], path: list[str]) -> list[str]:
        ...
        for candidate, order in orders.items():
            ...
            if len(parts) != len(path):
                continue
            if all(p in (WILDCARD, actual) for p, actual in zip(parts, path, strict=True)):
                return order
...
class FileProcessor:
    ...
    def process(self, path: Path) -> Result:
        ...
        sortable = [
            (doc, kind) for doc, kind in typed if kind != GENERIC and self.config.has_template(kind)
        ]
        if not sortable:
...
class SortingTool:
    def __init__(
        ...
        *,
        ...
        self.auditor = MissingKeyAuditor(self.config, substitution_markers or {"ALL_CAPS"})
        self.dry_run = dry_run
    ...
    def _report(self, stats: Stats) -> None:
        ...
        for file_type, keys in sorted(stats.missing_keys.items()):
            log.info(
                "keys absent from %s%s: %s", file_type, TEMPLATE_SUFFIX, ", ".join(sorted(keys))
            )
